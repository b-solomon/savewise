...
### 2. 🤖 Human-in-the-Loop AI Advisor (Safe Agent Execution)
...
- **Hardened CSV Statement Importer**: Includes **formula-injection protection** (neutralizing leading `=`, `+`, `-`, `@` characters), strict MIME checking, row/column boundaries, and automatic column detection.
...
- **Token Revocation Store**: Supports active user logout and immediate JWT revocation via an in-memory TTL blacklist.
- **Origin Isolation**: Enforces explicit CORS domain whitelisting in production.
...
- **Engineered Zero-Knowledge Financial Architecture**: Designed an end-to-end encryption engine using AES-256-GCM and PBKDF2 (100k iterations) separating JWT auth secrets from a dedicated master key, ensuring financial ledgers remain unreadable at rest.
- **Implemented Safe AI Agent Execution Flow**: Architected a Human-in-the-Loop confirmation gateway for LLM tool executions, eliminating prompt-injection write vectors by requiring cryptographically verified user approval before mutating database records.
- **Hardened Production Node.js / Express Backend**: Deployed Helmet security headers, CORS origin whitelisting, tiered rate limiting (`express-rate-limit`), and a JWT revocation blacklist for instant session invalidation.
- **Built Resilient Multi-Bank Ingestion Pipeline**: Developed bank SMS regex parsers and a robust CSV statement importer featuring automatic header mapping and formula injection protection (`=`, `@`, `+`, `-`).
- **Designed Resilient Hybrid Database Layer**: Integrated PostgreSQL with automatic fallback to SQLite and implemented a CI/CD automated diagnostic pipeline executing 25+ automated health and security checks on every pull request.
